...
public class XContentTypeOptionFilterTest {
	private HttpServletRequest mockRequest;
	private HttpServletResponse mockResponse;
	private FilterChain mockFilterChain;
	private XContentTypeOptionFilter testInstance;
	
	@Test
	public void shouldNoModifyTheResponeWhenXXSSProtectionIsBlock() throws Exception {
		//Given
		givenTestInstance();
		when(mockResponse.getHeader(eq("X-Content-Type-Options"))).thenReturn("nosniff");
		//When
	...
	public void shouldInjectTheResponeWhenXXSSProtectionIsNull() throws Exception {
		...
		assertThat(headerCaptor.getValue(), is(equalTo("X-Content-Type-Options")));
		assertThat(valueCaptor.getValue(), is(equalTo("nosniff")));
		verify(mockFilterChain).doFilter(eq(mockRequest), 
				eq(mockResponse));
	}
	...
	public void shouldInjectTheResponeWhenXXSSProtectionIsOther() throws Exception {
		//Given
		givenTestInstance();
		when(mockResponse.getHeader(eq("X-Content-Type-Options"))).thenReturn("allow");
		//When
		testInstance.doFilter(mockRequest, mockResponse, mockFilterChain);
		//Then
		ArgumentCaptor<String> headerCaptor = ArgumentCaptor.forClass(String.class);
	...
	private void givenTestInstance() {
		...
		testInstance = new XContentTypeOptionFilter();
	}
}
